...
// ---- 全息卡官方卡面（2026-09-22 二次实锤）------------------------------------
// fut.gg 同一个球员有两套卡面，**按端点分叉**：
//   · 列表接口 cardImagePath      = 2027/futgg-player-item-card/…  fut.gg 自绘「平版」（一直存的）
//   · definition-data cardImagePath = 2027/player-item-card/…       EA 官方卡面
// 全息卡的官方面（金色签名体 + 镭射环）在**变体 item** 上，数据模型见 scripts/holo.js。
// fetch_ci.js 把该路径叠进临时字段 holoCardImagePath（与数据库同名字段），
// 再由本表把它下载成 `{基础球员 eaId}_holo.webp` —— 文件名刻意用**基础球员**的 eaId：
// 端上 utils/format.js#getImg(p,'holo') 就是按 p.eaId 拼的，变体 eaId 只用于取图、不参与命名。
